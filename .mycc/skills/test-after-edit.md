...
description: >
  Remind to run pnpm test before committing, if code changes
  were made after the last test run. Uses lastIndexOf for ordering-aware checks.
when: block git_commit if NOT in plan mode, edit_file or write_file was used this session, and the last edit is newer than the last pnpm test run, requiring tests to pass before commit can proceed
---
