...
**session / turn / move:** **session** = full conversation history from agent start to exit (`Sequence.totalEventsCount`, `seq.totalCount()`). **turn** = from one user query to the next (begins at prompt submit, ends when agent returns to PROMPT; `Sequence.events` and most `seq.*` functions operate here, cleared at each `markPromptBoundary()` in PROMPT). **move** = a single LLM response including all tool calls in its delta (tool calls batched; hooks evaluate against prior moves in the same turn, not sibling tools in the same delta).
...
Skills that actively trigger based on patterns in the conversation sequence (vs. passive skills loaded on explicit request). Defined with a `when:` field in YAML frontmatter. Condition language queries conversation history: `seq.has(toolName)`, `seq.hasAny([tool1, tool2])`, `seq.lastIndexOf(pattern)` (supports `"bash#pattern"`), `seq.lastError()`, `seq.sinceEdit()`, `seq.totalCount(toolName?)`, `seq.isPlanMode()`. Actions: `inject_before`, `inject_after`, `block`, `replace`, `message`, `compact`.
