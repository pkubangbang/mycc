...
4. **1-2 questions per round.** No more. Use `ctx.core.question()` for structured choices; use plain text for open-ended questions.
...
3. **Load learner profile** (cross-topic memory):
   ```bash
   test -f "sigma/learner-profile.md" && echo "exists"
   ```
   If exists: read `sigma/learner-profile.md` with `read_file`. Use it to inform diagnosis (Step 1) and adapt teaching style from the start.
...
**If no level**: Ask 2-3 diagnostic questions using `ctx.core.question()`.
...
4. **Generate visual roadmap** → `roadmap.html`
   ...
   - Open in browser: `open roadmap.html` (Linux/macOS) or `start roadmap.html` (Windows)
...
Alternate between:
- **Structured questions** via `ctx.core.question()` — for multiple choice, code prediction
...
Ask learner self-assessment via `ctx.core.question()` BEFORE revealing evaluation:
...
2. Update `sigma/learner-profile.md` with cross-topic insights
...
4. Use `brief()` to report session summary to user
...
- **Pedagogy**: Bloom's 2-Sigma effect, Socratic Method, spaced repetition,
  interleaving practice, fluency illusion, misconception handling
- **Excalidraw**: Use `@excalidraw/excalidraw` via ESM CDN for concept maps
- **HTML visuals**: Single self-contained `.html` files, dark theme, glassmorphism,
  no external dependencies except CDN for Excalidraw
...
- [ ] Used `brief()` to report progress to user
- [ ] Used `ctx.core.question()` for interactive Q&A
