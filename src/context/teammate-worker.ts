...
function createPersistentTriologue(name: string, assignedPath?: string): TriologueLite {
  ...
  // Skill keywords are NO LONGER injected here as a project-context
  ...
  // a system-message input, so the LLM selects relevant keywords from the
  // actual available list based on the X+Y+Z composite. Teammates don't run
  // the COLLECT state, so they don't do skill discovery via extractKeywords;
  // task-focused teammates receive skill needs from the lead's HINT-driven
  // discovery.
