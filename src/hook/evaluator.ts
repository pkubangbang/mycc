...
function evaluateNode(node: jsep.Expression, ctx: EvalContext): unknown {
  switch (node.type) {
    ...
    case 'Identifier': {
      ...
      // Handle 'call' identifier - return call context or safe defaults
      if (name === 'call') {
        if (ctx.call) {
          return ctx.call;
        }
        // Return safe defaults when no call context available
        // This allows conditions like call.metadata.filePath.includes() to not crash
        // and fail safely (empty string for filePath, 0 for loc values)
        return {
          metadata: {
            filePath: '',
            newLoc: 0,
