 ...
 * (`matchesBaselineGate`), the predicate that replaces both the old noisy
 * `Z > 0` bidirectional-substring `includes` filter and the reverted κσ
 * hypergeometric adaptive gates.
 *
 ...
 *
 * These tests pin the behavior of the pure helper in isolation — the
 * integration tests in collect-keyword-integration.test.ts cover the real
 * handleCollect() path.
 */
import { describe, it, expect } from 'vitest';
import { matchesBaselineGate } from '../../../loop/states/collect-skill.js';
...
describe('matchesBaselineGate — exact-token intersection (no substring)', () => {
  it('matches on an exact shared token', () => {
    ...
    // Z = 1 >= 0.6 → match.
    expect(matchesBaselineGate(['go', 'rust', 'python'], ['go'], 10)).toBe(true);
  });

  it('does NOT match on a substring ("go" must not match "logging")', () => {
    // The reverted substring `includes` filter let "go" match "logging"; the
    // exact-token intersection kills that. Z = 0 → no match.
    expect(matchesBaselineGate(['logging', 'debug'], ['go'], 10)).toBe(false);
  });
  ...
  it('does NOT match on a superstring ("logging" must not match "log")', () => {
    ...
    expect(matchesBaselineGate(['go', 'run'], ['logging'], 10)).toBe(false);
  });

  it('matching is case-insensitive on both sides', () => {
    expect(matchesBaselineGate(['Go', 'Rust'], ['GO', 'rust'], 10)).toBe(true);
  });
});
...
describe('matchesBaselineGate — the baseline bar', () => {
  it('demands more overlap as the query grows (Y - 1 scales the cap)', () => {
    ...
    const skill = ['a', 'b', 'c'];
    expect(matchesBaselineGate(skill, ['a', 'b'], 12)).toBe(true);
    expect(matchesBaselineGate(skill, ['a', 'b', 'c', 'd'], 12)).toBe(true);
    expect(matchesBaselineGate(skill, ['a', 'b', 'c', 'd', 'e'], 12)).toBe(true);
  });
  ...
  it('rejects when overlap is below the E[Z] proportionality bar', () => {
    ...
    expect(matchesBaselineGate(skill, ['a', 'b', 'c', 'x', 'y'], 10)).toBe(false);
    expect(matchesBaselineGate(skill, ['a', 'b', 'c', 'd', 'x'], 10)).toBe(true);
  });
  ...
  it('Y - 1 caps the demand so a fat skill cannot self-promote on tiny overlap', () => {
    ...
    // though E[Z] is huge. This is the property the reverted ρ rule tried to
    // enforce; the cap does it without a separate rule.
    const skill = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'];
    expect(matchesBaselineGate(skill, ['a', 'x', 'y', 'z', 'q'], 10)).toBe(false); // Z=1
    expect(matchesBaselineGate(skill, ['a', 'b', 'c', 'd', 'x'], 10)).toBe(true);  // Z=4
  });
  ...
  it('is permissive for a small skill in a large universe', () => {
    ...
    expect(matchesBaselineGate(['niche', 'rare'], ['niche'], 100)).toBe(true);
  });
});
...
describe('matchesBaselineGate — Y clamp [2, 5]', () => {
  it('clamps a 1-keyword query up to Y=2 (so Y-1=1 keeps a meaningful bar)', () => {
    ...
    // Z = 1 → match.
    expect(matchesBaselineGate(['go', 'rust'], ['go'], 10)).toBe(true);
  });
  ...
  it('clamps a huge query down to Y=5 (so Y-1=4 does not dominate small W)', () => {
    ...
    expect(matchesBaselineGate(skill, query, 10)).toBe(true);
  });
});

describe('matchesBaselineGate — edge cases', () => {
  it('keywordless skill (X=0) never matches', () => {
    expect(matchesBaselineGate([], ['go', 'rust'], 10)).toBe(false);
  });

  it('zero overlap (Z=0) never matches', () => {
    expect(matchesBaselineGate(['a', 'b'], ['x', 'y'], 10)).toBe(false);
  });
  ...
  it('W=0 (no universe) falls back to the Z > 0 floor', () => {
    ...
    expect(matchesBaselineGate(['go', 'rust'], ['go'], 0)).toBe(true);
    expect(matchesBaselineGate(['go', 'rust'], ['x'], 0)).toBe(false);
  });

  it('W=1 falls back to the Z > 0 floor (no degenerate bar)', () => {
    // W=1: expected = (X/1)*Y could be huge; the floor path avoids that.
    expect(matchesBaselineGate(['go'], ['go'], 1)).toBe(true);
    expect(matchesBaselineGate(['go'], ['x'], 1)).toBe(false);
  });

  it('Z >= ceil-less baseline works on the continuous threshold', () => {
    ...
    // raw `Z >= baseline`, NOT `Z >= ceil(baseline)` (which the reverted
    // κσ code used and which over-suppressed at small W).
    expect(matchesBaselineGate(['a', 'b', 'c'], ['a'], 10)).toBe(true); // baseline 0.6, Z=1
  });
});