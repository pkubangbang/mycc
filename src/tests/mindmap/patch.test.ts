...
describe('patch_mindmap', () => {
  ...
  describe('Cascade to Ancestors', () => {
    it('should update ancestor summaries', () => {
      ...
      // All ancestor summaries should be regenerated by the mock
      // generateSummary → `Summary: <title> (level <level>)`. The ancestor
      // titles/levels are unchanged, so assert the exact regenerated values
      // (not just "defined") to prove the cascade actually ran.
      ...
      expect(newChildSummary).toBe('Summary: Child (level 2)');
      expect(newParentSummary).toBe('Summary: Parent (level 1)');
  ...
  describe('Cascade to Descendants', () => {
    it('should update descendant summaries when parent changes', () => {
      ...
      // Child and grandchild summaries should be regenerated by the mock
      // generateSummary (titles/levels unchanged → exact values).
      const child = findNode(mindmap.root, '/parent/child');
      const grandchild = findNode(mindmap.root, '/parent/child/grandchild');

      expect(child?.summary).toBe('Summary: Child (level 2)');
...
describe('summarize_node', () => {
  ...
  it('should generate summary for a node', () => {
    ...
    // Mock generateSummary returns `Summary: <title> (level <level>)`.
  ...
  it('should update node summary in place', () => {
    ...
    // The node's summary is overwritten with the regenerated value.
    expect(node?.summary).toBe('Summary: Child (level 2)');
  });
});
