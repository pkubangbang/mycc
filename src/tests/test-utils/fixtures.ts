...
export const contextFactories = {
  ...
  fileOps: (workdir: string): AgentContext => ({
    core: {
      ...
      brief: () => {},
      verbose: () => {},
      question: async () => ({ question: '', answer: '', reason: '', source: 'user' as const }),
      webSearch: async () => [],
      webFetch: async () => ({ title: '', content: '', links: [], url: 'https://example.com' }),
      imgDescribe: async () => '',
      readPictureCached: async () => ({ pairs: [], cacheToken: 'token' }),
      requestGrant: async () => ({ approved: true }),
      requestExternalPathAccess: async (_tool, path) => ({ approved: true, resolvedPath: path }),
      addExternalAutoGrant: () => {},
      ...
      resetConfusionIndex: () => {},
      escAware: (async (fn: (ac: AbortController) => Promise<unknown>) => fn(new AbortController())) as never,
    },
    todo: {} as never,
    mail: {} as never,
    skill: {} as never,
