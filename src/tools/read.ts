...
import { fileHandlers } from './read-handlers.js';
...
export const readTool: ToolDefinition = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      aloud: {
        type: 'boolean',
        description: 'If true, also display the file content to the terminal user via brief (in addition to returning it here). Use this when the user explicitly asks to see or "replay" a file\'s content (e.g. a crossroad record). Default: false.',
      },
    },
    required: ['path'],
  },
...
${typeInfo.extension === '.png' || typeInfo.extension === '.jpg' || typeInfo.extension === '.jpeg' || typeInfo.extension === '.gif'
  ...
  : 'Use bash tool with appropriate program to process this file.'}`;
      ...
      // A registered handler (see read-handlers.ts) may post-process the raw
      // content before it reaches the LLM. This short-circuits ALL of the
      // truncation/header/preview logic below: when a handler matches, its
      // result replaces what read_file returns, and (when aloud=true) the
      // handler's displayText is shown to the terminal via brief instead of
      // the raw content.
      //
      // The crossroad-json-handler uses this to implement the replay bypass:
      // it merges prefix + continuation into displayText and returns only a
      // short confirmation as toolResult, so the full reconstructed text
      // reaches the terminal WITHOUT being regenerated by the LLM (and thus
      // outside detectTurningWord's scan scope).
      for (let i = 0; i < fileHandlers.length; i++) {
        const handler = fileHandlers[i];
        if (handler.matches(resolved, content)) {
          const { toolResult, displayText } = handler.process(resolved, content);
          if (aloud) {
            ctx.core.brief('info', 'read_aloud', displayText);
          }
          ctx.core.brief('info', 'read', `${filePath} (handled: handler #${i + 1})`);
          return toolResult;
        }
      }
      ...
      // Build the result string. There are four mutually exclusive text paths:
      // long-lines preview, line-limit truncation, char-limit truncation, and
      // the full-file read. Each assigns `result` rather than returning, so the
      // single aloud brief at the bottom can display exactly what the LLM gets.
      let result: string;
      if (hasLongLines) {
...
To de-minify, run in bash: npx prettier --write ${filePath}  (for JS/TS/JSON/CSS)`;
        ...
        result = `${header}\n${previewParts.join('\n')}${suggestions}`;
      } else if (totalLines > LINE_LIMIT) {
        // Apply hard-coded line limit
...
${remainingLines} more lines not shown. To continue:
  ...
  • read_read — summarize with a focus topic (if file in .mycc/longtext/)`;
        ...
        // File fits within line limit; apply char-limit safety net if needed
      ...
      // ── aloud display (no handler matched) ──
      // When the user asked to see the content (aloud=true) but no FileHandler
      // claimed the file, brief the exact result the LLM receives so the
      // terminal shows the same text (header, truncation markers, and all).
      // The handler-matched path already briefed its displayText above.
      if (aloud) {
        ctx.core.brief('info', 'read_aloud', result);
      }
